#!/usr/bin/env python3
"""
Script simplifié pour mesurer les temps de chargement via docker exec
"""
...
def execute_mysql_query(query):
    """Exécuter une requête MySQL via docker exec"""
    cmd = [
        'docker', 'exec', 'geneticvariantsdb-db-1',
        'mysql', '-uusr', '-pusrpass', 'vardb',
        '-e', query
    ]
    result = subprocess.run(cmd, capture_output=True, text=True)
    return result.stdout

def measure_sample_loading(sample_name, regions):
    """Mesurer le temps de chargement pour un échantillon"""
    
    # Échapper les apostrophes dans le nom de l'échantillon
    sample_escaped = sample_name.replace("'", "\\'")
    
    # Créer la liste des variants pour la requête IN
    regions_str = "','".join([r.replace("'", "\\'") for r in regions])
    
    # 1. Mesurer le temps de requête pour compter les variants
    ...
    WHERE r.name = '{sample_escaped}'
    AND v.name IN ('{regions_str}');
    """
    ...
    try:
        ...
        if len(lines) > 1:
            variant_count = int(lines[1])
        else:
            variant_count = 0
    except:
        variant_count = 0
    
    # 2. Simuler le calcul des statistiques (temps d'agrégation)
    ...
    if variant_count > 0:
        ...
        AND v.name IN ('{regions_str}');
        """
        execute_mysql_query(stats_query)
...
def main():
    ...
    for i, sample in enumerate(samples, 1):
        try:
            result = measure_sample_loading(sample, regions)
            results.append(result)
            
            # Affichage de la progression
            print(f"[{i:3d}/{len(samples)}] {sample[:50]:50s} | "
            ...
            print(f"[{i:3d}/{len(samples)}] {sample[:50]:50s} | ERREUR: {str(e)}")
            results.append({
                'sample_name': sample,
                'variant_count': 0,
    ...
    print(f"Temps total d'exécution: {total_elapsed:.2f} secondes ({total_elapsed/60:.2f} minutes)")
